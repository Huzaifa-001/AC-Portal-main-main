...
import { CoreModule } from "../../../core/core.module"; // Ensure correct import
  ...
  staticEstimates = [
    {
      ...
      signed: false,
      items: [
        { itemName: 'Item 1', description: 'Description 1', quantity: 2, price: 100, amount: 200 },
      ],
      subtotal: 200,
      tax: 20,
      total: 220,
  ...
  submitForm(): void {
    if (this.estimateForm.valid) {
      ...
      if (this.updateData.Request_Type === 'Add') {
        ...
        const index = this.staticEstimates.findIndex((e) => e.id === estimate.id);
        if (index > -1) {
      ...
      this.toastr.success(
        `${this.updateData.Request_Type === 'Add' ? 'Added' : 'Updated'} Successfully`,
        'Success'
      );
