...
export class CreditMemoService {
  ...
  getAllCreditMemos(relatedValue: string, pageIndex: number, pageSize: number): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}?relatedValue=${relatedValue}&pageIndex=${pageIndex}&pageSize=${pageSize}`);
  }

  // Add a new credit memo
  addCreditMemo(creditMemo: CreditMemoDto): Observable<CreditMemoDto[]> {
  ...
  updateCreditMemo(creditMemo: CreditMemoDto): Observable<CreditMemoDto[]> {
    return this.http.put<CreditMemoDto[]>(`${this.apiUrl}/${creditMemo.creditMemoNumber}`, creditMemo);
  }

  // Delete a credit memo
  deleteCreditMemo(creditMemoNumber: string): Observable<CreditMemoDto[]> {
    return this.http.delete<CreditMemoDto[]>(`${this.apiUrl}/${creditMemoNumber}`);
  }
}
