...
export class BudgetsService {
  ...
  deleteBudget(budgetNumber: string): Observable<BudgetDto[]> {
    // Perform the delete action
    this.staticData = this.staticData.filter(
      (budget) => budget.budgetNumber !== budgetNumber
    );
    return of(this.staticData);
  }

  // 3. Update an estimate
  updateBudget(updatedBudget: BudgetDto): Observable<BudgetDto[]> {
    const index = this.staticData.findIndex(
      (budget) => budget.budgetNumber === updatedBudget.budgetNumber
    );
    if (index > -1) {
  ...
  addBudget(newBudget: BudgetDto): Observable<BudgetDto[]> {
